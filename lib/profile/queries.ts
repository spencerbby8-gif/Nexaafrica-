...
/**
 * Replace a user's parsed profile in a single transaction-ish flow.
 * Skills and experience are wiped and re-inserted to keep the data clean.
 */
export async function saveParsedProfile(userId: string, parsed: ParsedProfile) {
  const supabase = await createClient()
...
export async function markProfileStatus(
  ...
  if (status === 'failed' && errorMsg) {
    await supabase
      ...
      .upsert(
        { profile_id: userId, last_error: errorMsg.slice(0, 500), last_run_at: new Date().toISOString() },
