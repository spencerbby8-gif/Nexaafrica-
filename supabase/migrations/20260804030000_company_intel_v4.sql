-- Nexa Intelligence V4 — Company Intelligence expansion (2026-08-04)
-- Richer company profile: long-term hiring behavior + hiring consistency.
-- All values are measured from real job history — nothing fabricated.
-- (Response quality has no data source on the platform yet — omitted honestly.)
...
alter table public.company_intelligence
  ...
  add column if not exists active_months int not null default 0,
  add column if not exists distinct_months int not null default 0,
  add column if not exists remote_friendliness real not null default 0;

-- V4 company aggregation (replaces V3 — same name, richer output).
...
as $$
  ...
  agg AS (
    SELECT
      ...
      -- Active months: calendar months spanned by postings (>=1)
      count(DISTINCT date_trunc('month', b.posted_at))::bigint AS distinct_months,
      (count(*) FILTER (WHERE b.remote_ok))::real / NULLIF(count(*), 0)::real AS remote_friendliness
    FROM base b
    GROUP BY b.company
  )
  SELECT a.company, a.total_jobs, a.africa_eligible_jobs, a.remote_jobs, a.rejected_jobs,
    a.dead_page_count, a.verified_count, a.africa_rate, a.rejection_rate,
    a.verification_rate, a.trust_avg, a.hiring_velocity_30d, a.salary_consistency,
    a.duplicate_count, a.avg_ai_confidence,
    coalesce(r.scam, 0)::bigint AS scam_reports,
    a.first_posted_at, a.last_posted_at,
    -- active_months: full calendar months from first to last posting (min 1)
    ...
    a.distinct_months, a.remote_friendliness
  FROM agg a
  LEFT JOIN (
